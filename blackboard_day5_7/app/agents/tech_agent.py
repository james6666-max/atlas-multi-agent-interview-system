...
TECH_QUESTION_TYPES = {"technical", "algorithm", "system_design"}


class TechAgent(Agent):
    name: ClassVar[str] = "tech_agent"
    ...
    async def handle(self, event: BBEvent) -> List[BBEvent]:
        ...
        if normalized_type not in TECH_QUESTION_TYPES:
            return []
        if not question:
            return [self._event(event, EventType.ERROR, {"question": "", "error": "TechAgent received empty question"})]
        ...
        try:
            ...
            self.logger.exception("TechAgent failed to run Phase 1 pipeline")
        ...
        return [
            self._event(
                ...
                EventType.ANSWER_DRAFT,
                {
                    ...
                    "selected_agent": result.get("selected_agent", "Tech/Code"),
                    "draft": answer,
                    "answer": answer,
                    "raw_result": result,
                    "context": context,
