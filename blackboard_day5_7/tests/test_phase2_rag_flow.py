...
def ask_payload(question: str, selected_agent: str, context: dict | None, rag: dict | None) -> dict:
    return {
        "question": question,
...
def test_technical_flow_passes_context_and_rag_to_tech_agent(monkeypatch) -> None:
    calls = []
    ...
    async def fake_pipeline(
        ...
        calls.append((agent_hint, context, rag))
    ...
    assert calls and calls[0][0] == "tech"
    assert calls[0][1]["has_resume"] is True
    assert calls[0][2]["has_rag"] is True
    assert "knowledge.txt" in calls[0][2]["sources"]


def test_behavioral_flow_passes_rag_but_keeps_resume_constraints(monkeypatch) -> None:
    calls = []

    monkeypatch.setattr(
        "app.agents.rag_agent.load_knowledge_text",
        lambda: "Atlas project used FastAPI, React, Electron, Ollama, Whisper, and OCR.",
    )
    ...
    assert response.status_code == 200
    assert calls and calls[0][0] == "behavioral"
    assert any("Use only resume/JD/knowledge facts" in item for item in calls[0][1]["constraints"])
    assert any("STAR" in item for item in calls[0][1]["constraints"])
    assert calls[0][2]["has_rag"] is True


def test_chitchat_does_not_trigger_rag_or_answer_agents(monkeypatch) -> None:
    def fail_rag():
        raise AssertionError("RAGAgent should not run for ignored input")
    ...
    monkeypatch.setattr("app.agents.rag_agent.load_knowledge_text", fail_rag)
...
def test_empty_rag_result_does_not_prevent_answer(monkeypatch) -> None:
    calls = []
    monkeypatch.setattr("app.agents.rag_agent.load_knowledge_text", lambda: "")
    monkeypatch.setattr("app.agents.rag_agent.retrieve_session_history", lambda question, events, top_k=3: [])
    ...
    assert response.status_code == 200
    assert calls and calls[0]["has_rag"] is False
    assert response.json()["rag_used"] is False


def test_rag_agent_error_does_not_prevent_answer(monkeypatch) -> None:
    calls = []
    ...
    monkeypatch.setattr("app.agents.rag_agent.load_knowledge_text", broken_rag)
    monkeypatch.setattr(phase1_pipeline, "run_phase1_answer_pipeline", fake_pipeline)
    client = TestClient(orchestrator_v0.app)
